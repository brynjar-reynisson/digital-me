...
- The free tier's ~15 requests/minute ceiling can be exceeded by a handful of searches in quick succession, since the frontend summarizes up to 5 results concurrently per search — any request that hits the ceiling gets a 429, which `GeminiSummarizeClient` treats like any other failure and falls back to `DeepseekSummarizeClient`, same as today's behavior before this feature existed, just triggered more often under rapid searching
